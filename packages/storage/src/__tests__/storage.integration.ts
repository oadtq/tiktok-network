...
async function runTests() {
  ...
  try {
    ...
    console.log(`❌ Test 1: generateKey threw error: ${e instanceof Error ? e.message : String(e)}`);
    failed++;
  }
  ...
  try {
    ...
    console.log(`❌ Test 2: getPublicUrl threw error: ${e instanceof Error ? e.message : String(e)}`);
    failed++;
  }
  ...
  try {
    ...
    if (result.key === key && result.size === testData.length) {
      ...
      console.log(`❌ Test 3: upload failed. Result: ${JSON.stringify(result)}`);
      failed++;
    }
  } catch (e) {
    console.log(`❌ Test 3: upload threw error: ${e instanceof Error ? e.message : String(e)}`);
    failed++;
  }
  ...
  try {
    ...
    console.log(`❌ Test 4: getPresignedUploadUrl threw error: ${e instanceof Error ? e.message : String(e)}`);
    failed++;
  }
  ...
  try {
    ...
    const result = await storage.uploadVideo("test-user", "test-clip.mp4", videoData);
    
    if (result.key.includes("clips/test-user") && result.key.includes("test-clip.mp4")) {
    ...
    console.log(`❌ Test 5: uploadVideo threw error: ${e instanceof Error ? e.message : String(e)}`);
    failed++;
  }
